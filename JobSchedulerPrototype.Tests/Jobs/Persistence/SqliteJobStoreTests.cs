...
public sealed class SqliteJobStoreTests
{
    ...
    public async Task TryClaimNextDueJobClaimsScheduledJobOnlyWhenRunAtArrives()
    {
        ...
        var earlyClaim = store.TryClaimNextDueJob(scheduledAt.AddTicks(-1), "worker-1");
        var dueClaim = store.TryClaimNextDueJob(scheduledAt, "worker-1");
    ...
    public async Task TryClaimNextDueJobIgnoresTerminalJobEvenWhenRunAtIsPresent()
    {
        ...
        store.Add(queuedJob);
        store.TryClaimNextDueJob(now, "worker-1");
        ...
        var claimedJob = store.TryClaimNextDueJob(now, "worker-1");
    ...
    public async Task TryClaimNextDueJobClaimsEachDueJobOnceAcrossConcurrentWorkers()
    {
        ...
        var workers = Enumerable
            ...
            .Select(_ => Task.Run(() =>
            {
                while (true)
                {
                    var claimedJob = database.CreateStore().TryClaimNextDueJob(now, "worker-1");
